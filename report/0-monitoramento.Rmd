...
pedidos_cgu <- "data/base-cgu.rds" %>%
  ...
  select(id_pedido, data_registro, data_resposta, prazo, ts_registro, ts_resposta, decisao, orgao)
```
...
### Tempo de resposta dos pedidos - em dias corridos (2012-2022)
...
pedidos_prazo <- pedidos_cgu %>% 
  ...
  transmute(
    ...
    dias_corridos = dmy(ts_resposta) - dmy(ts_registro),
    ...
    prazo = lubridate::dmy(prazo)
  )
```

```{r fig.height=10}
pedidos_prazo %>% 
  count(name = 'n_pedidos_ano_prazo', dias_corridos_fx, ano = year(data_registro), sort = TRUE) %>%
  ...
  scale_fill_manual(values = cores) +
  scale_y_percent(expand = c(0, 0), limits = c(0,.5)) +
